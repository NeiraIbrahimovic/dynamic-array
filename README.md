...
A Java resizable array implementation with indexed access and element removal.

## Technical depth

Capacity management, indexing, and collection edge cases.
...
Use the API in MyArrayList.java; this is a library rather than an interactive application.
...
The repackaged source compiled with JDK 24. The local JUnit run passed **24 tests**, with 0 failures. Maven configuration is provided for convenience; the reported run used javac and JUnit Console directly. Passing tests do not establish exhaustive correctness or production readiness.
